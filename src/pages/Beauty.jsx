...
const BeautyPage = () => {
  ...
  // Converts "Salon for Women" to "salon_for_women" and navigates to the dynamic service page
  const handleServiceClick = (title) => {
    // Customize this path if you are linking to your Hub page vs your Service page!
    // Example: If it contains "salon", maybe route to '/hub/womens_salon', else route to '/service/...'
  ...
  // --- MOCK DATA (Unchanged) ---
  const heroServices = [
    { title: "Salon for Women", img: "https://www.urbancompany.com/img?bucket=urbanclap-prod&quality=90&format=auto/w_56,dpr_2,fl_progressive:steep,q_auto:low,f_auto,c_limit/images/supply/customer-app-supply/1774421563473-192084.jpeg", tag: "Sale", time: "45 mins", name:"salon_for_women" },
    { title: "Spa for Women", img: "https://www.urbancompany.com/img?bucket=urbanclap-prod&quality=90&format=auto/w_56,dpr_2,fl_progressive:steep,q_auto:low,f_auto,c_limit/images/supply/customer-app-supply/1774421566139-db72a8.jpeg", tag: "Sale", name:"ayurvedic_spa_women" },
    { title: "Hair Studio for Women", img: "https://www.urbancompany.com/img?bucket=urbanclap-prod&quality=90&format=auto/w_56,dpr_2,fl_progressive:steep,q_auto:low,f_auto,c_limit/images/growth/luminosity/1728839468364-90b0dc.jpeg",name:"hair_studio_women" },
    { title: "Makeup, Saree & Styling", img: "https://www.urbancompany.com/img?bucket=urbanclap-prod&quality=90&format=auto/w_56,dpr_2,fl_progressive:steep,q_auto:low,f_auto,c_limit/images/growth/luminosity/1669023257508-ffd582.jpeg",name:"makeup_saree_styling" },
    { title: "Salon Prime for Men", img: "https://www.urbancompany.com/img?bucket=urbanclap-prod&quality=90&format=auto/w_56,dpr_2,fl_progressive:steep,q_auto:low,f_auto,c_limit/images/growth/luminosity/1710241114433-5cfa7c.jpeg",name :"salon_prime_men" },
    { title: "Massage for Men", img: "https://www.urbancompany.com/img?bucket=urbanclap-prod&quality=90&format=auto/w_56,dpr_2,fl_progressive:steep,q_auto:low,f_auto,c_limit/images/supply/customer-app-supply/1674623814769-eeca92.jpeg", name:"prime_massage_men" }
  ];
        ...
        <div className="max-w-[1200px] mx-auto px-4 md:px-8 pt-10 pb-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-10">
            <div>
              ...
              <div className="border border-[#2A4334]/10 bg-white rounded-2xl p-6 shadow-sm mb-10">
                ...
                <div className="grid grid-cols-3 gap-y-8 gap-x-2">
                  {heroServices.map((service, idx) => (
                    // 3. ATTACH THE ONCLICK TO HERO SERVICES
                    <div key={idx} onClick={() => handleServiceClick(service.name)} className="flex flex-col items-center group cursor-pointer">
              ...
              {/* ... Statistics div remains unchanged ... */}
          ...
          <div className="flex overflow-x-auto gap-4 pb-4 hide-scrollbar snap-x">
            {promoBanners.map((banner, idx) => (
              <div
                key={idx}
                // 3. ATTACH THE ONCLICK TO PROMO BANNERS
                onClick={() => navigate('/service/special-promo')}
          ...
          <HorizontalSlider title="Most booked services">
            {mostBooked.map((item, idx) => (
              // 3. ATTACH THE ONCLICK TO MOST BOOKED
              <div key={idx} onClick={() => handleServiceClick(item.title)} className="snap-start flex-none w-[180px] cursor-pointer group">
          ...
          <HorizontalSlider title="Salon for women">
            {salonForWomen.map((item, idx) => (
              // 3. ATTACH THE ONCLICK TO SALON FOR WOMEN
              <div key={idx} onClick={() => handleServiceClick(item.title)} className="snap-start flex-none w-[180px] cursor-pointer group">
          ...
          {/* 5. LARGE BANNER (Spa) */}
          <div onClick={() => navigate('/service/spa-special')} className="w-full h-[250px] md:h-[380px] rounded-2xl overflow-hidden relative mt-12 mb-16 cursor-pointer group shadow-sm border border-[#2A4334]/5">
          ...
          <HorizontalSlider title="Spa for women" subtitle="Refresh. Rewind. Rejuvenate.">
            {spaForWomen.map((item, idx) => (
              // 3. ATTACH THE ONCLICK TO SPA FOR WOMEN
              <div 
                key={idx} 
          ...
          <section className="mt-16 mb-16">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10">
              {hairAndNailGrid.map((item, idx) => (
                // 3. ATTACH THE ONCLICK TO GRID ITEMS
                <div key={idx} onClick={() => handleServiceClick(item.title)} className="cursor-pointer group flex flex-col">
          ...
          <div onClick={() => navigate('/service/spa-package')} className="w-full h-[250px] md:h-[380px] rounded-2xl overflow-hidden relative mt-12 mb-16 cursor-pointer group shadow-sm border border-[#2A4334]/5">
          ...
          <HorizontalSlider title="Salon for Men">
            {salonForMen.map((item, idx) => (
              // 3. ATTACH THE ONCLICK TO SALON FOR MEN
              <div key={idx} onClick={() => handleServiceClick(item.title)} className="snap-start flex-none w-[180px] cursor-pointer group">
          ...
          <div onClick={() => navigate('/service/massage-package')} className="w-full h-[250px] md:h-[380px] rounded-2xl overflow-hidden relative mt-12 mb-16 cursor-pointer group shadow-sm border border-[#2A4334]/5">
          ...
          <HorizontalSlider title="Massage for Men" subtitle="Refresh. Rewind. Rejuvenate.">
            {spaForMen.map((item, idx) => (
              // 3. ATTACH THE ONCLICK TO MASSAGE FOR MEN
              <div 
                key={idx} 
                onClick={() => handleServiceClick(item.title)}
                className="snap-start flex-none w-[220px] md:w-[240px] h-[280px] md:h-[300px] bg-white border border-[#2A4334]/10 rounded-2xl overflow-hidden cursor-pointer flex flex-col group hover:shadow-lg hover:border-[#AA593E]/30 hover:-translate-y-1 transition-all duration-300"
